...
a chain of ten or more digits joined by any short run of punctuation (a phone number in whatever
shape, and anything longer), a run of four or more digits joined by a space, hyphen, dash, slash,
underscore or middle dot, and four or more spoken digits in a row, because a voice transcript spells
numbers out.
...
groups of `416/555/0199` (the Phase 8 gate reviews). The chain cannot be the only rule, because it
...
# Up to three characters that are neither a letter, a digit nor a dollar sign, between two digits.
...
_DIGIT_RUN = re.compile(r"\d(?:[\s\-\u2010-\u2015/_\u00b7]?\d){3,}")
