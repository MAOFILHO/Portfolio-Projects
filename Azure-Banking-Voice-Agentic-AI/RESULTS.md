...
| **B4** | cost ceiling, fails closed | 0 overruns, 0 fail-open | **0 observed.** The one known undercount (a cancelled ledger write) is closed in code and tested, **not yet live**. | `tests/`, `docs/PLAN.md` |
...
| # | criterion | result |
|---|---|---|
| 1 | redacted transcript to Blob, no raw write first | Met. Unit-proved; live on **three** calls. No blob for a closed-line call or one with no agent turns (nothing to store); one agent turn over 1,000 characters fails closed (a row, no transcript). |
| 2 | summary/intent/outcome row for every completed call | Met. Live on **three** calls. The row is now written first (`pending`) and overwritten when the steps finish, so a kill mid-pipeline leaves the pending row (tested, **not yet live**). |
| 3 | five-value outcome enum | Met (D17). |
| 4 | pipeline cannot affect B4 or B5 | Met by construction (background task, never awaited). The test proves the same frames and turn cap; it **measures no latency**. The gate review also moved every async Azure client onto one async credential, so a token refresh no longer blocks the event loop (tested, **not yet live**). |
...
- **The gate review's fixes are tested, not yet live** (they need one new image and one real call): one async
  credential for every Azure client, the row written before the slow steps, the shielded daily-ledger write,
  and a phone scrub that masks every punctuated shape (`(416) 555-0199` and `416.555.0199` used to leave the
  area code, or six digits, behind).
