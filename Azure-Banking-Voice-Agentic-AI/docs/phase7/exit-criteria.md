...
| Container Apps environment | **`infra/modules/container-apps-env.bicep`**, added 2026-09-16 | none. Values match live (`az containerapp env show`, 2026-09-16) — Consumption workload profile only, no VNet, logs bound to `...1D`. Still not deployed. |
| Voice-agent Container App | **`infra/modules/voice-agent.bicep`**, added 2026-09-16 | none. Marco's call, 2026-09-16: no separate "voice-agent" app exists or ever existed — `ca-azbank-echo-p0` already is the production caller-facing app, so this module models on its live shape (`az containerapp show`) and keeps its live name. Still not deployed. |
