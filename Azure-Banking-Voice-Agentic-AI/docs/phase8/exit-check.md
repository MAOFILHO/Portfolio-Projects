...
Nothing in this table is a projection: where a criterion rests on a live call, it says how many. The
gate-review fixes are tested but **not yet live**: they need one new image and one real call.
...
| Live | three real calls: two on image `p8a`, one on `p8b` (revision `ca-azbank-echo-p0--0000002`). `p8b` predates the gate-review fixes. |
...
| # | criterion | evidence |
|---|---|---|
| 1 | Redacted transcript reaches Blob, no raw write first | ✅ Unit tests prove the blob writer only receives redacted text and that a failed redaction writes nothing (ADR-007). **Live on three calls**: each blob read back, no PIN, phone number or digit run. **Limits:** no blob for a closed-line call or one with no agent turns (nothing to store, so `transcript_status=none`); one agent turn over 1,000 characters fails closed and keeps its row. |
| 2 | A summary/intent/outcome row for every completed call | ✅ Live on three calls, each `caller_hangup`, transcript `stored`, summary `done`. A call whose realtime connection cannot be opened also gets a row (tested on the main and closed paths, not live). The row is now written **first** with `pending` fields and overwritten when the steps finish, so a kill mid-pipeline leaves the pending row (tested, **not live**). |
...
| 9 | `RESULTS.md`, an architecture diagram, a current README badge | ✅ `RESULTS.md` (77 lines, ceiling 120); `docs/architecture.md` (Mermaid, parsed clean by the mermaid parser, not viewed rendered on GitHub); README badge "Phase 8 of 8 (exit gate pending)". |
...
Fixed, tested, and (where marked) **held for your look** because the change is on B2, B4 or a billable resource:

| finding | disposition |
|---|---|
| `scrub_numbers` left the area code, or six digits of a dotted number | **Fixed.** A phone-shaped pattern runs first; `$12.50` and `4.25 percent` survive. **B2, held.** |
| A cancelled ledger write lost that call's minutes (B4) | **Fixed.** The write is its own shielded task; shutdown waits for it. **B4 / `session.py`, held.** |
| Row written last; a kill loses it | **Fixed.** Row first with `pending`, overwritten at the end. |
| Sync credential on the event loop (B5 exposure, and the ledger client since Phase 5) | **Fixed.** One async credential. |
...
| No Bicep for Language or the text deployment | **Written, never deployed, not wired into `azbank-deploy`.** A read-only `what-if` found one deploy blocker: a duplicate Language role assignment. **Billable IaC, held.** |
...
1. **Your look** at the held diffs (B2 scrub, B4 shield, the Bicep and the B3 script change).
2. **One new image and one real call** to prove the fixes live (needs `az containerapp update`, yours to run).
