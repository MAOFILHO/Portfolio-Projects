"""State management for the Tool Orchestrator module.

Resets and reloads the Postgres backend before each task, per Ticket 02.
Both services (control and pattern) share this one class since they front
the same backend.
"""
...
class ToolOrchestratorStateManager(BaseStateManager):
    def __init__(self):
        super().__init__(service_name="tool_orchestrator")
