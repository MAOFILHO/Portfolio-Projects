...
class MCPStdioServer:
    ...
    async def list_resources(self) -> List[Dict[str, Any]]:
        resp = await asyncio.wait_for(self.session.list_resources(), timeout=self.timeout)
        return [r.model_dump() for r in resp.resources]

    async def list_resource_templates(self) -> List[Dict[str, Any]]:
        resp = await asyncio.wait_for(self.session.list_resource_templates(), timeout=self.timeout)
        return [t.model_dump() for t in resp.resourceTemplates]

    async def read_resource(self, uri: str) -> Any:
        result = await asyncio.wait_for(self.session.read_resource(uri), timeout=self.timeout)
