...
class BaseMCPAgent(ABC):
    ...
    def _build_read_resource_tool(
        ...
        """Describe a server's MCP resources as one synthetic tool.

        Native tool-calling APIs have no separate "resource" primitive, so a
        server's resources are surfaced as one more tool the model can call
        by name, listed in its description. Template resources (a URI like
        "runbook://{id}", not independently listable) are described too, with
        a note to fill in the placeholder -- the model gets the concrete id
        from another tool. Returns None if the server exposes neither, so
        servers without any are unaffected.
        """
        if not resources and not resource_templates:
            return None
        lines = [
            f"- {r['uri']}: {r.get('description') or r.get('name', '')}" for r in resources
        ] + [
            f"- {t['uriTemplate']} (replace the {{placeholder}} with a real id from another tool): "
            f"{t.get('description') or t.get('name', '')}"
            for t in resource_templates
        ]
        return {
            "name": self.READ_RESOURCE_TOOL_NAME,
            "description": (
                "Read the content of one MCP resource by its URI. Available resources:\n"
                + "\n".join(lines)
            ),
            "inputSchema": {
                "type": "object",
    ...
    async def _augment_tools_with_resources(
        self, mcp_server: Any, tools: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """Fetch a server's MCP resources (concrete and template) and append
        the synthetic read_resource tool describing them, if it has any.
        Shared by all 3 agent call sites so the fetch-and-append shape lives
        in one place."""
        resources = await mcp_server.list_resources()
        resource_templates = await mcp_server.list_resource_templates()
        read_resource_tool = self._build_read_resource_tool(resources, resource_templates)
        return tools + [read_resource_tool] if read_resource_tool else tools

    async def _dispatch_tool_call(
        self, mcp_server: Any, name: str, arguments: Dict[str, Any]
    ) -> Any:
        """Route a model's tool call to the right MCP client method:
        read_resource for the synthetic resource-reading tool, call_tool for
        everything else."""
        if name == self.READ_RESOURCE_TOOL_NAME:
            return await mcp_server.read_resource(arguments.get("uri", ""))
