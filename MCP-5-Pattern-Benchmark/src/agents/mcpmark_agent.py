...
class MCPMarkAgent(BaseMCPAgent):
    ...
    async def _execute_claude_native_with_tools(
        ...
        async with mcp_server:
            ...
            tools = await self._augment_tools_with_resources(mcp_server, tools)
    ...
    async def _execute_anthropic_native_tool_loop(
        ...
        for _ in range(max_turns):
            ...
            for tu in tool_uses:
                ...
                try:
                    result = await asyncio.wait_for(
                        self._dispatch_tool_call(mcp_server, name, inputs), timeout=60
                    )
                    tool_results.append(
                        {
                            "type": "tool_result",
    ...
    async def _execute_litellm_tool_loop(
        ...
        try:
            while turn_count < max_turns:
                ...
                # Moonshot/Kimi-specific extensions; other providers (e.g. OpenAI)
                # reject unrecognized top-level request arguments.
                if self.litellm_input_model_name.startswith("moonshot/"):
                    completion_kwargs["enforcer_mode"] = "on"
                    completion_kwargs["think_mode"] = "on"
                ...
                if hasattr(message, "tool_calls") and message.tool_calls:
                    ...
                    for tool_call in message.tool_calls:
                        ...
                        try:
                            result = await asyncio.wait_for(
                                self._dispatch_tool_call(mcp_server, func_name, func_args),
                                timeout=60,
                            )
                            messages.append(
                                {
    ...
    def _create_stdio_server(self) -> MCPStdioServer:
        ...
        elif self.mcp_service == "wrapper":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.tool_orchestrator.server_wrapper"],
            )

        elif self.mcp_service == "orchestrator":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.tool_orchestrator.server_orchestrator"],
            )

        elif self.mcp_service == "domain_wrapper":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.tool_orchestrator.server_wrapper"],
            )

        elif self.mcp_service == "domain_adapter":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.domain_adapter.server_domain_adapter"],
            )

        elif self.mcp_service == "session_baseline":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.stateful_session.server_baseline"],
            )

        elif self.mcp_service == "session_server":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.stateful_session.server_session"],
            )

        elif self.mcp_service == "proxy_wrapper":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.tool_orchestrator.server_wrapper"],
            )

        elif self.mcp_service == "proxy_aggregator":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.proxy_aggregator.server_proxy_aggregator"],
            )

        elif self.mcp_service == "resource_wrapper":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.tool_orchestrator.server_wrapper"],
            )

        elif self.mcp_service == "resource_gateway":
            return MCPStdioServer(
                command=sys.executable,
                args=["-m", "src.mcp_services.resource_gateway.server_resource_gateway"],
            )
