...
| 2026-08-19 | **Criterion 4's deploy gate: `make verify-lambda-execution` (11/13, 2 known-open, unrelated) + full `C1` harness against `CodeSha256 MX//FPM7wEq+bQNgNoFmsIaShb/FuSsNtQYDnJT8Sx8=`** — `e7763ff`'s `PHONE_RE` fix. Composed recall **1.000 (26/26)**, 0 contingency, 0 unstable, false escalations 9/17 (unchanged from every prior run). `RESULTS.md` §96, `APPROVED: Phase 11` (this entry) | **Yes** | `verify-lambda-execution`: 10 guardrail/router events + 1 generation call (same basis as the two 2026-08-16 rows above). `C1` harness: 95 `RecognizeText` requests (78 positive-path + 17 negative) | **~$0.0032 (verify-lambda-execution, estimate) + $0.098007 (C1 harness, exact) ≈ $0.101207** |

**Phase 11 running total, this table:** $0.01454570 + $0.0032 + $0.0003 + $0.097668 + $0.101207 ≈ **$0.2170**.
...
| 2026-08-16 | 11 (claim (b)) | **Claim (b)'s live check — one real `lambda:Invoke`** against `fnol-codehook`, `UpdateContactInfo`/`field=email` confirmation-readback turn, real `ApplyGuardrail` both sides (INPUT: 2 content+topic units; OUTPUT: 2 content+topic units + 1 sensitive-information unit) plus one real router `Converse` call (Nova Micro, short prompt) — `RESULTS.md` §76. Not separately metered against a per-request rate the way CE calls are; estimated from the guardrail pricing table ($0.15/1k content+topic, $0.10/1k sensitive-info) plus `verify-lambda-execution`'s own per-event router-call estimate | **Yes** | ~$0.0007 (guardrail) + ~$0.0003 (router, estimated) ≈ $0.001 | **~$0.051** |
| 2026-08-16 | 12 (Block 1) | **`D121`'s input-side question — two real `ApplyGuardrail` OUTPUT calls**, `BedrockGuardrailClient`, live `zl5ppnyorwd2` v5, placeholder text `"That's {EMAIL} -- is that right?"` and the `{PHONE}` equivalent — a negative control, not live input-side verification; both returned `action: NONE` — `RESULTS.md` §78. Exact `usage`: 2 calls × (`topicPolicyUnits:1 + contentPolicyUnits:1` @ $0.15/1000 + `sensitiveInformationPolicyUnits:1` @ $0.10/1000, 0 free units) | **Yes** | $0.0008 exact (2 × $0.0004) | **~$0.0518** |
| 2026-08-16 | 12 (Block 2) | **`D121` Block 2 pre-decision probes — eight real `ApplyGuardrail` OUTPUT calls**, `BedrockGuardrailClient`, live `zl5ppnyorwd2` v5, testing Marco's spelled/phonetic/grouped-digit readback preference (6 full-value variants, all masked) plus two partial-disclosure follow-ups (email prefix passed clean, phone last-4 still masked) — `RESULTS.md` §79. Same per-call composition as the row above | **Yes** | $0.0032 exact (8 × $0.0004) | **~$0.0550** |
| 2026-08-17 | 12 (Block 2, `/grill-with-docs` Round 3) | **`ADR-017`'s free-text-PII audit — rate probe, scoped in Round 2, run this entry.** Seven real `generate_response()` calls (default generation model `us.amazon.nova-lite-v1:0`, `PROMPT-REGISTRY.md` §3.1's real `_COVERAGE_SYSTEM_PROMPT`, a static synthetic policy-text stand-in for `search()`'s retrieved text, one control + 6 `coverage_topic` variants embedding EMAIL/PHONE-shaped text at start/middle/end) followed by seven real `ApplyGuardrail` OUTPUT calls (`BedrockGuardrailClient`, live `zl5ppnyorwd2` v5) against each real `answer`. Result: **0/6 PII-embedded probes had the caller's email/phone echoed verbatim in the model's generated answer** — the existence question (already answered by the code trace) needed no guardrail intervention to observe on this sample; the guardrail read `action: NONE` on all 7. Script: `coverage_topic_pii_probe.py`, session scratchpad, one-off, not the permanent gate. Converse: ~2,450 input + ~175 output tokens across 7 calls ≈ $0.0002 (Nova Lite, $0.06/$0.24 per 1M). Guardrail: 7 × ($0.15/1000 content+topic × 2 units + $0.10/1000 sensitive-info × 1 unit, 0 free units) = 7 × $0.0004 = $0.0028. Approved by Marco at "the `§79` order of magnitude" before running | **Yes** | $0.0002 (Converse) + $0.0028 (guardrail) ≈ $0.0030 | **~$0.0580** |
| 2026-08-17 | 12 (Block 2, Round 3 Q1) | **Same 6 PII-embedded `coverage_topic` probes, re-run against the flagged ALTERNATE generation model** (`us.anthropic.claude-haiku-4-5-20251001-v1:0`, not the live default) — Marco's instruction, confirmatory before treating the rate as characterized. Same call shape (real `Converse` via the real wrapped client, then real `ApplyGuardrail` OUTPUT on the answer); `generate_response()` itself takes no model override, so this script replicates its exact call construction directly against `ALTERNATE_GENERATION_MODEL_ID` rather than flipping the live flag. Script: `coverage_topic_pii_probe_alt_model.py`. Result: **0/6 again** — combined **0/12 across both flaggable models**. Converse: Claude Haiku 4.5 is priced far higher than Nova Lite ($1.00/$5.00 per 1M vs. $0.06/$0.24) — ~2,100 input + ~210 output tokens across 6 calls ≈ $0.0021 + $0.00105 = $0.0032. Guardrail: 6 × $0.0004 = $0.0024. A third script, `coverage_check.py` ($0 — pure regex, no AWS calls), then ran `EMAIL_RE`/`PHONE_RE` against all 13 real outputs from both runs (nothing to catch, matching 0/12) and directly against the two PII fixtures (Marco's check 1, below and in `ADR-017`) | **Yes** | $0.0032 (Converse) + $0.0024 (guardrail) ≈ $0.0056 | **~$0.0636** |
| 2026-08-17 | 12 (Block 2, `ADR-017` condition part 3) | **First real `make redteam` run by that literal name** (`D126`/`OI49` — the target didn't exist before this entry) **plus `ADR-017`'s new readback probe**, one combined process, `python -m redteam.run --guardrail-id zl5ppnyorwd2 --guardrail-version 5` (version read live, `aws bedrock list-guardrails`, immediately before running, per Marco's instruction — not assumed from any doc). Two components, split by `CostLog`'s own before/after snapshot (printed separately by `run.py`, not estimated): **(a) the pre-existing 11-attack corpus**, unchanged, 7 real `Converse` calls, 1,424 in / 134 out, **$0.0001176** — matches the 2026-08-12 row above to the digit, as expected for an unchanged deterministic corpus against the same models; result 11/11 defended, unchanged. Guardrail-call cost for this half not itemized, consistent with how the 2026-08-12 row itself was logged (Converse only). **(b) The new readback probe** (`redteam/readback_probe.py`): 7 discovered dynamic sites across the four non-exception `OUTPUT_GUARDRAIL_SOURCES` node modules, all 7 covered (zero coverage gaps), all 7 real `ApplyGuardrail` OUTPUT calls returned **`action: NONE`** — **the class direction 3-coarse leaves open is confirmed checked and currently clean**, not merely wired. Two real `generate_response` calls (`coverage_question`, `rental_towing`), 1,404 in / 77 out, **$0.00010272** (by subtraction from the combined post-probe total, `$0.00022032`, `docs/evidence/redteam-report-readback-probe.json`). Guardrail: all 7 calls read clean (no PII/content/topic intervention), so each is *assumed* the same per-call composition as the `Block 1`/`Block 2`/`Round 3` rows above (`topicPolicyUnits:1 + contentPolicyUnits:1` @ $0.15/1000 + `sensitiveInformationPolicyUnits:1` @ $0.10/1000, 0 free units) = 7 × $0.0004 = **$0.0028 — FORMULA-DERIVED, NOT MEASURED.** This run's own `usage` blocks were never captured (`ReadbackProbeReport.as_dict()` dropped `SiteProbeResult.guardrail_usage` on the floor at call time — a real reporting gap, not a rounding choice), so this figure is inferred from the same per-call shape prior *measured* rows established, not read off this run's own response. **Fixed in the same change, not retroactively**: `as_dict()` now includes `guardrail_usage` per site, so the *next* `make redteam` run will report its guardrail cost exactly, the same way the Converse figures in this row already are. Full per-site table (`site_id`, real `response_text`, `action`) in the session report below, not just pass/fail | **Yes** | (a) $0.0001176 exact + (b) $0.00010272 exact (Converse, both measured) + $0.0028 (guardrail, **formula-derived, not measured — see note**) ≈ **$0.00312** | **~$0.0667** |
| 2026-08-21 | 12 (row 9, `D160`/`OI78`) | **`terraform apply "row9.tfplan"`, `APPROVED: Phase 12`.** `0 added, 1 changed, 0 destroyed` (JSON action tally: 22 `no-op`, 1 `update`, 2 `forget`). `aws_lambda_function.codehook` updated in-place — `CodeSha256` `MX//FPM7...` → `q9mbvGOnTmWI2T1hhbiGQy7bTRczQZOVHg1rEFCcoh4=` (confirmed live via `aws lambda get-function`, not the plan's claim), shipping `92c8800`'s `D140`/`OI58` escalation fix, nothing past it (`git diff --stat 92c8800 HEAD -- src/` empty). `aws_lambda_layer_version.codehook_deps` and `aws_s3_object.codehook_deps_layer` detached from Terraform management via `removed { lifecycle { destroy = false } }` blocks (`D160`/`OI78`'s data-source pin) — confirmed `forget`, not `destroy`, both in the plan JSON and live: `Layers` on the deployed function still reads `arn:...:layer:fnol-codehook-deps:2`, `CodeSize 43849548`, unchanged. `forget` is a pure state-file operation, no AWS API call | Yes (1 Lambda control-plane update; the 2 `forget`s made zero AWS calls) | 1 `UpdateFunctionCode`/`UpdateFunctionConfiguration` call | **$0.00** | **~$0.0667** |
| 2026-08-21 | 12 (row 9, mandatory `C1` re-verification) | **`C1` re-verification against `CodeSha256 q9mbvGOnT...`, `scripts/measure_composed_pipeline_deployed.py`, full protocol — the blanket "any `stacks/main` code change requires re-verifying `C1`" rule (`RESULTS.md` §24), triggered by this deploy shipping `92c8800`. NOT row 9's own Layer 1 bar** — this instrument drives the injury/fatality-escalation trigger surface (the sixth in-scope intent), a different surface from the three `D140`/`OI58` sites (INPUT-guardrail-block, OUTPUT-guardrail-block, `UpdateContactInfo` confirm-ceiling), which `PROJECT_STATE.md` §100 is explicit still needs its own, not-yet-built harness. Composed recall **1.000 (26/26)**, 0 contingency items, 0 unstable items, no per-item divergence from D52. 9/17 negatives false-escalated (0.529) — **checked item-by-item against `evals/baselines/composed_pipeline_k5_v3_20260812.json`'s own `composed_worst_case` negatives: the exact same 9 texts, not merely the same count** — confirms this is the same already-recorded characteristic every prior run of this instrument has shown (2026-08-14, -15, -16 rows above), not a new regression introduced by the `D140`/`OI58` fix or the `D160`/`OI78` layer-reference change | **Yes** | 95 `RecognizeText` requests (78 positive-path + 17 negative), graph-path Bedrock/guardrail calls per the script's own cost basis | **$0.098684** (lex $0.07125 + bedrock $0.027434) | **~$0.1654** |
| 2026-08-21 | 12 (row 9, Layer 1 live check, `scripts/verify_row9_layer1_escalation_wire.py`) | **4 real `recognize_text` calls against the deployed bot, existing committed harness (`dc4c770`), explicit distinct `session_id`s, raw responses captured to `evals/baselines/row9_layer1_*.json`.** Site 1 (INPUT-guardrail-block, `"yes, go ahead and file it"`): escalated, wire confirmed. Site 3 (`UpdateContactInfo` `_CONFIRM_CEILING`, multi-turn, one session): turn 1 succeeded (elicited `policy_number`); **turn 2 (`"PY4821"`) raised a live `DependencyFailedException`** — a genuine, previously-unknown live-only defect, sequence could not complete, no escalation established. Site 2 (attempted OUTPUT-guardrail-block via a `CoverageQuestion`-shaped legal/settlement phrasing): call succeeded but landed on the **same INPUT-guardrail-block branch as site 1** (confirmed structurally — `sessionState.intent.name: "FallbackIntent"` in the raw response, meaning the INPUT guardrail's pre-routing check fired before any intent-specific node ran — not established from message-text matching alone). Site 2's OUTPUT-block branch remains unestablished this run | **Yes** | 4 `RecognizeText` requests (1 site 1 + 2 site 3 [1 completed, 1 errored] + 1 site 2), of which at least 3 reached the graph (guardrail ran) per `sessionAttributes`/`executed_node_intent` evidence | **$0.003 exact (Lex, 4 × $0.00075)** + **~$0.001–0.0014 estimated** (Bedrock/guardrail, same per-call order of magnitude as `C1`'s measured rate — **not cross-checked against CloudWatch this run, unlike `C1`'s own harness**) ≈ **~$0.004–0.0044** | **~$0.1695–0.1698** |
| 2026-08-28 | 12 (`D89`/`OI6` fix, Option B — `FileAutoClaim` confirmation prompt reword "file"→"submit") | **Live-guardrail verification of the reworded prompt, `scripts/verify_d89_submit_wording.py` (new), against `zl5ppnyorwd2` v5 (confirmed live via `list-guardrails` immediately before running, not assumed).** 2 runs, 11 real `ApplyGuardrail` INPUT calls each (6 fix-set + 4 regression-set + 1 informational). Run 1: usage not captured (script instrumented for exact usage between runs). Run 2, exact from `response.usage`: `topicPolicyUnits:11, contentPolicyUnits:11`, both runs' pass/fail results identical (deterministic). Result: **6/6 fix-set phrases NONE** (the "submit" prompt and its natural affirmative replies no longer trigger `legal_and_medical_advice`); **4/4 regression-set still BLOCKED** (genuine legal-advice phrasing unaffected — the reword did not weaken the topic); the known pre-existing medical-example gap (`RESULTS.md` §47/§49, out of this fix's scope) reproduces identically. `RESULTS.md` §101 | **Yes** | 22 units/run (11 topic + 11 content) × 2 runs = 44 units total; run 2 exact, run 1 formula-derived from the identical call shape | **$0.0066** (2 × $0.0033 @ $0.15/1k for 22 units) | **~$0.1764–0.1830** |
| 2026-08-28 | 12 (Step 2 — `FileAutoClaim` success response: recap + claim number + next steps) | **Live-guardrail verification of the new success-response text, `scripts/verify_d89_submit_wording.py` extended (sets 4-5)**, against `zl5ppnyorwd2` v5 (confirmed live via `list-guardrails` immediately before running). Set 4: the real success text (`_recap_for_success` + claim number + `_NEXT_STEPS`, built from the live module, not hand-copied), tested on **source="OUTPUT"** — the actual channel it runs through in production (`file_auto_claim` is a real `OUTPUT_GUARDRAIL_SOURCES` member, `agents/graph.py:88-90`), not INPUT like the D89 sets. Set 5: 4 natural caller closing replies ("ok, thanks" etc.), source="INPUT". Exact `usage`: `topicPolicyUnits:16, contentPolicyUnits:16, sensitiveInformationPolicyUnits:1` (the 5-set combined run; sets 1-3 repeated from the row above plus the 2 new sets) — the nonzero sensitive-information unit corroborates `CLAUDE.md`'s own documented fact that this policy evaluates on OUTPUT, unlike INPUT (always 0 there). Result: **success response NONE on OUTPUT; all 4 natural replies NONE on INPUT** | **Yes** | 16 topic + 16 content + 1 sensitive-info units (1 combined run covering all 5 sets) | **$0.0049** (32 units × $0.15/1k + 1 unit × $0.10/1k) | **~$0.1813–0.1879** |
| 2026-08-29 | 12 (`D89`/`D162`/`D140`/Step-2 combined deploy) | **`terraform apply "d89-d162-demo.tfplan"`, `APPROVED: Phase 12 demo`.** `0 added, 1 changed, 0 destroyed` (JSON action tally: 22 `no-op`, 1 `update`, confirmed against the plan JSON directly, not the human summary alone). `aws_lambda_function.codehook` updated in-place — `CodeSha256` `Pqlr2UtdB5bFRdanLF04lpClzS77HVL/+xNX38z89ck=` → `b9PDFWWySU/UlOT2h17ml8i3PpDk3o/pw0r/1hHETec=`, confirmed live via `get-function-configuration` post-apply, not the apply output alone. `Layers` unchanged (`fnol-codehook-deps:2`, `CodeSize 43849548`) — the layer detachment held. `did_routed = false` in the apply's own outputs, confirming `route_did` untouched | Yes (1 Lambda control-plane `UpdateFunctionCode`/`UpdateFunctionConfiguration` call) | 1 update call | **$0.00** | **~$0.1879** |
| 2026-08-29 | 12 (mandatory `C1` re-verification against the new `CodeSha256`) | **`scripts/measure_composed_pipeline_deployed.py`, full protocol, against `CodeSha256 b9PDFWWySU/...`.** Composed recall **1.000 (26/26)**, 0 contingency items, 0 unstable items, no per-item divergence from D52's local verdicts. 9/17 negatives false-escalated — checked against the known pre-existing characteristic (`RESULTS.md`'s prior runs), not a new regression. `C1` restored to VERIFIED against the new hash | **Yes** | 95 `RecognizeText` requests (78 positive-path + 17 negative), exact per-call CloudWatch attribution (positives) + assumed graph-path (negatives, per the script's own documented method) | **$0.097668** exact (lex $0.07125 + bedrock $0.026418) | **~$0.2856** |
| 2026-08-29 | 12 (`make verify-lambda-execution`, 13-event gate, run twice to check determinism on an unexpected failure) | **2 full runs of the 13-event gate against the new deploy.** Both runs identical: **10/13**, 3 failures — event 2 (`CheckClaimStatus first turn`, **NEW**, see `D200`/`OI118` below), event 12 (`FileAutoClaim filed, all slots pre-filled` — `D89`/`OI6`'s caller-initiated-phrasing residual, confirmed still firing via the exact `transcript="yes, go ahead and file it"` the test drives, matching the residual named at this fix's own commit), event 13 (`RentalTowingEntitlement fulfilled` — `D90` part 1, known, unrelated, unchanged). Cost is the script's own **printed pre-run estimate** (not a post-hoc measured figure the way `C1`'s script computes it) — labeled as such, not presented as exact, per this project's own "estimated vs. exact" convention | Yes (10 of 13 events reach real Bedrock per run) | ~10 guardrail/router calls + 1 generation call per run, × 2 runs | **~$0.0064** (2 × the script's own ~$0.0032 estimate — NOT independently measured) | **~$0.2920** |
| 2026-08-29 | 12 (`D200`/`OI118` fix — `terraform apply "d200.tfplan"`, cost gate treated as satisfied per Marco's instruction as this session's own combined-deploy follow-up, no new `APPROVED:` line typed for this specific apply) | **`terraform -chdir=infra/terraform/stacks/main plan -out=d200.tfplan` shown before applying: exactly `0 to add, 1 to change, 0 to destroy`, the sole change `aws_lambda_function.codehook`'s `source_code_hash` moving off `b9PDFWWySU/UlOT2h17ml8i3PpDk3o/pw0r/1hHETec=` — matched the predicted shape exactly, no layer resource, no create/destroy anywhere. Applied; confirmed live via `get-function-configuration`: `CodeSha256 sf41EXFv61SdZdvoorZ96qI8JlUTtHR5tQoDZhv/gVU=`, `Layers` unchanged `fnol-codehook-deps:2`. `did_routed = false` in the apply's own outputs, `route_did` untouched | Yes (1 Lambda control-plane `UpdateFunctionCode`/`UpdateFunctionConfiguration` call) | 1 update call | **$0.00** | **~$0.2920** |
| 2026-08-29 | 12 (`D200`/`OI118` fix, mandatory `C1` re-verification against the new `CodeSha256`) | **`scripts/measure_composed_pipeline_deployed.py`, full protocol, against `CodeSha256 sf41EXFv...`.** Composed recall **1.000 (26/26)**, 0 contingency items, 0 unstable items, no per-item divergence from D52's local verdicts. 9/17 negatives false-escalated — same known pre-existing characteristic as every prior run, not a new regression from this deploy. `C1` restored to VERIFIED against the new hash | **Yes** | 95 `RecognizeText` requests (78 positive-path + 17 negative), exact per-call CloudWatch attribution (positives) + assumed graph-path (negatives, per the script's own documented method) | **$0.098684** exact (lex $0.07125 + bedrock $0.027434) | **~$0.3907** |
| 2026-08-29 | 12 (`D200`/`OI118` fix, `make verify-lambda-execution`, 13-event gate, run twice — first run matched the 11/13 bar, second run surfaced one additional, distinct, nondeterministic failure investigated below) | **Run 1: 11/13** (up from 10/13) — event 2 (`CheckClaimStatus first turn`) now **passes**, confirming `D200`'s fix live; events 12/13 fail as expected (`D89`/`D90`, unchanged, see rows above). **Run 2: 10/13** — events 12/13 fail identically, PLUS a third, new failure at event 3 (`CoverageQuestion first turn`: `dialogAction={'type':'Delegate'}` instead of `ElicitSlot`). Root-caused via CloudWatch (`errorMessage`): `_UnroutableIntentError: active_slot='coverage_topic' but result['intent']=<Intent.AMBIGUOUS: 'Ambiguous'> declares no Lex slots` — the router classified this turn `Ambiguous` (a real, if rare, low-confidence classification on this transcript) while the `coverage_question` node's own `active_slot="coverage_topic"` still reached `_elicit_slot`; the guard correctly refused to send an illegal `ElicitSlot` under a non-slot-bearing declared intent — the guard did its job, this is a live router-classification/graph-state interaction, not a defect in this session's `D200` fix (which touches only `CheckClaimStatus`'s legal-slot set) and not reproduced in run 1. **Also confirmed via CloudWatch, not just the test's own message**: event 12's block is real (`guardrail_usage {"blocked": true, ... "source": "INPUT"}` on the exact `requestId` for `"yes, go ahead and file it"`), and event 13's live misroute target is `graph_intent=FileAutoClaim` (confirmed via the `turn ...` log line, both runs, deterministic) — **this differs from `D90`/`OI7`'s own on-record misroute target (`CoverageQuestion`)**, flagged as new data in `PROJECT_STATE.md`, not silently reconciled. Cost is the script's own printed pre-run estimate, labeled as such | Yes (10-11 of 13 events reach real Bedrock per run) | ~10-11 guardrail/router calls + 1 generation call per run, × 2 runs | **~$0.0064** (2 × the script's own ~$0.0032 estimate — NOT independently measured) | **~$0.3971** |
