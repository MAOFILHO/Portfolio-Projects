...
def test_independent_holdout_set_is_absent_until_stage_6() -> None:
    """Criterion 14's set is generated by an isolated agent at Stage 6, not authored inline here --
    authoring it in this file would defeat its entire purpose, since this file's author wrote the
    lexicon. This test documents the absence as intentional so it does not read as an oversight, and it
    is expected to be inverted (to an assertion of presence) when Stage 6 generates the set."""
    from evals.holdout import HoldoutKind, HoldoutSetMissingError, load_holdout

    with pytest.raises(HoldoutSetMissingError):
