...
| **8** ✅ | **Real-call verification** — one `classify_turn` call through the real assembled graph, `CoverageQuestion`'s optional-election generation path, `RentalTowingEntitlement`'s compound path, plus `CF3`'s 5 real Nova Micro tight-turn samples. Found a real, recurring defect (`RentalTowingEntitlement`'s redundancy-via-restatement fix reduces but doesn't reliably eliminate the issue) and a real test-hygiene bug (`mock_aws()` intercepts *all* boto3 traffic in its scope, not just the service it's meant to fake) | Stage 4, 6, 7 | **Yes — the only stage that spent anything.** ≈$0.00025 combined across two passes, logged in `COSTS.md` |
