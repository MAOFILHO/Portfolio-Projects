...
| 5 | Eval-gate workflow — deployment status | **Corrected 2026-08-15 — was overstated.** "Landed... verified byte-identical" was true only of two local copies; `origin/main` was pinned at `a4d8ae6` (2026-08-12) throughout, so the file has never existed on the branch GitHub reads — "not yet exercised by a push/PR" understated it, there was no commit on GitHub for a trigger to be missing from. `workflow_dispatch` added 2026-08-15; deployed copy re-synced, re-verified, committed locally (`7a5d6f0`); **push to `origin/main` blocked (tool-permission denial) and scope-expanded to 76 unpushed local commits** — Marco's decision on how to proceed, not made here | `RESULTS.md` §12.1, §12.6 |
