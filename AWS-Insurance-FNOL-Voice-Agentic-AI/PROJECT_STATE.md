...
`PROJECT_STATE.md`'s own backlog row 3 (`:8454`) — none of which point at anything that
...
|---|---|---|---|
| 1 | **Filter, not raise.** `_elicit_slot` filters `lex_slots` to `graph_intent`'s legal key set (via `legal_slots_by_intent`) before embedding it in the response — illegal keys are silently dropped, no exception. Closes the **router-drift trigger's residual illegal keys** (the trigger actually observed live, turn 1→2, `d162-obs-site3-confirm-ceiling-recheck`). Test must assert against **`FileAutoClaim`'s own set as returned by `legal_slots_by_intent(bot.yaml.tftpl text)`** — never a hardcoded literal slot name. | `OI80` minimum-fix-set (b): "filter `lex_slots` to that same legal set before embedding it (closes the router-drift trigger's residual illegal keys)" (`:1026`) | NOT SHIPPED — no consumer of `legal_slots_by_intent` exists in `src/` |
| 2 | **Raise on illegal `slot_name`.** Extend the existing 3-part guard (`lex_codehook.py:398-412`) with a 4th check: `slot_name in legal_slots_by_intent[graph_intent]`, raising the existing `_UnroutableIntentError` if not. Closes the **`repair.py` stale-`active_slot` trigger** (`repair.py:43-72`'s `handle_no_match_or_barge_in` never touches `active_slot`, so a slot from a *prior*, different slot-bearing intent survives into a freshly, validly classified low-confidence turn). **Caller-facing behavior of the raise, traced, not assumed**: because `handle_no_match_or_barge_in`'s own retry key is `state.get("active_slot") or _UNKEYED_TURN` (`repair.py:44`) — the *same* stale slot the new check flags — the shared retry counter (`retry_ladder.py`, `RETRY_CEILING = 2`, `:13-15`) has already incremented under that key on every turn this check can fire, *before* `_elicit_slot` runs (`_run_graph_turn` commits its checkpoint at `lex_codehook.py:535-549`, before `_elicit_slot` is called at `:637`). Consequently: **turn N raises → no safety signal → silent `Delegate` echo (`handler:764-767`)**; nothing clears `active_slot` between turns, so **turn N+1 hits the same key's ceiling → `escalation` is set → `_respond_from_graph_result` checks `escalation` (`:617-618`) before the `active_slot`/`_elicit_slot` branch (`:635-637`) → `_elicit_slot` is never called turn N+1 → clean route-3 `_close(escalated=True, ...)`**. **Repeated delegates do not loop indefinitely — they resolve to escalation on the very next turn**, contingent on exactly one thing: the DynamoDB checkpointer's `graph.get_state(config)` read (`lex_codehook.py:704`, `:708`) succeeding each turn, so `retry_counts` carries forward rather than restarting at 0. If that read itself fails, `_dispatch` raises before the graph runs — a different, upstream, uncounted `Delegate` turn. Closing condition: **turn N (silent delegate) → turn N+1 (route-3 escalation) demonstrated live on the same 6-turn run row 4 captures**, read from `retry_counts`/`escalation_reason` on the wire, not inferred from source. | `OI80` minimum-fix-set (a) (`:1026`); trigger traced to `repair.py:43-72`; retry ladder traced to `retry_ladder.py:13-30`, `repair.py:44-45,47-69`; call-order traced to `lex_codehook.py:535-549,617-618,635-637,704-709,764-767` | NOT SHIPPED |
| 3 | **Derived `bot.yaml.tftpl` slot-legality mapping**, standalone, wired into `make lint`. | Commit `2124fd0` (Makefile diff): "the `{intent: {slot names}}` map derived from `bot.yaml.tftpl`'s own `Slots:` blocks, as a standalone check — **not yet consumed by `api/lex_codehook.py::_elicit_slot` (rows 1/2's own scope)**." `verify-slot-legality-mapping` target runs `scripts/verify_slot_legality_mapping.py --require-at-least 5`, wired into `lint:`. | **CLOSED** — `2124fd0` confirmed an ancestor of `HEAD` |
