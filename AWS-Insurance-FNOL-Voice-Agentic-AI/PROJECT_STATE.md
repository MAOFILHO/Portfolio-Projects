...
| Q12 | **The generation path still samples at temperature 0.7.** `ROUTER_TEMPERATURE = 0.0` (`D27`) pins the router only; `generate_response()` sends no `temperature`, so Nova Lite runs at Nova's default. Every §4 generation number is therefore still a single draw, and `CF5`'s "intermittent" redundancy defect is a direct symptom of it | Phase 7 verification stage | Deliberately **not** changed with the router fix: pinning it mid-phase would invalidate Phase 6's generation baselines, and whether a *spoken* response should be deterministic is a design question (a voice agent repeating a caller's phrasing verbatim on every retry reads as robotic) rather than a hygiene one. Decide it explicitly, with the redundancy detector as the evidence | 2026-08-12 |
