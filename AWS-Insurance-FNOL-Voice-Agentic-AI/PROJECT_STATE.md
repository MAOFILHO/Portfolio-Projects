...
**Progress 2026-08-19, criterion STILL OPEN — the local half of the second exit-evidence option is now done, the deployed half is explicitly not (`RESULTS.md` §95).** `D124`/`OI46` CLOSED on its own scope: `PHONE_RE` fixed (`[2-9]`-gated, not the old `555` literal), RED-first (both `scripts/verify_log_redaction.py` and `tests/unit/test_pii_redaction.py` captured failing against the unmodified pattern before the fix, exit 1, not skipped past), superset and false-positive claims verified explicitly against real fixture values, not assumed. Run 1 now exercises phone (`"416-987-1547"`), not only the synthetic email constant this row's prior text named as the gap. `OI47` converted from report to a standing check (`tests/unit/test_pii_redaction_generality.py`), so a future category this narrow fails loudly instead of passing on luck. **None of this is the deployed-runtime proof.** The fixed code has not been deployed; `OI2`'s own scope note stands unchanged (attachment only, never content); a Run 3 against the deployed Lambda, or an explicit accept-risk decision, is still the named exit evidence and was explicitly out of scope this session, per instruction. Criterion 4 remains **OPEN**. |
