...
| 11 | Observability and operations | 🟡 **APPROVED 2026-08-15.** Stage 0 (preflight) complete. Stage A (budget alarm + cost dashboard) applied 2026-08-15 — 12/12 resources live, verified against plan, no drift. **Criterion 1 CLOSED 2026-08-16** — full firing-proof chain confirmed (threshold applied → `ALARM` live → SNS published → breach email received and confirmed by Marco, `ACTUAL $0.71`); temporary test notification's removal plan ready, awaiting apply (`OI1`). Stage B1 (operational dashboard) applied and committed. **Criterion 8a CLOSED 2026-08-16** (`RESULTS.md` §74) — Lambda-invocation p95 1,651.06ms, 121 samples, scope-qualified. Remaining gaps: criterion 2 (cost-dashboard cross-check), criterion 3 claim (b) (OUTPUT-panel liveness probe), criterion 5 (two ops runbooks) — all open, none blocked on a `C1` cycle |
