...
def create_venv_and_install(python312_bin: str, name: str, venv_parent: Path, requirements_file: Path) -> None:
    ...
    print(f"\n=== {name} ===")
    if venv_python.exists() and not venv_pip.exists():
        # A prior run left a half-built venv (e.g. interrupted mid pip-install) —
        # `python` exists so the old check would wrongly treat this as "already set up".
        print(f"{name}: found a broken venv (missing bin/pip) — rebuilding it")
...
def main() -> int:
    ...
    for name, service_dir, _port in PYTHON_SERVICES:
        ...
        # Invoke it as `python -m pip`, not the `bin/pip` script directly — that script was just
        # regenerated by the `pip install --upgrade pip` above, and execing it again immediately
        # can transiently fail with FileNotFoundError before the new script is fully in place.
        venv_python = service_dir / ".venv" / "bin" / "python"
        subprocess.run([str(venv_python), "-m", "pip", "install", "pytest==8.3.4", "-q"], check=True)
    install_frontend()
